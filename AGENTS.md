...
5. If the active work clearly depends on unresolved recent multi-day context, also read additional recent daily notes as needed from the top level active window or `memory/daily/YYYY/MM/` for older canonical daily notes.
6. In the main session only, also read `MEMORY.md`.
